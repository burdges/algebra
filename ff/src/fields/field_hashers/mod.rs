...
/// ```
pub fn xmd_hash_to_field<H,F,const N: usize>(
    sec_param: u16,
    dst: &[u8],
    msg: &[u8]
) -> [F; N]
    ...
    H: FixedOutputReset + Default,
{
    ...
    let mut xmd = Zpad::<H>::new_for_field::<F>(sec_param)
        .chain(msg)
        .expand_for_field::<F,2>(sec_param,&dst);
    let h2f = |_| hash_to_field::<F,_>(sec_param, &mut xmd);
    ark_std::array::from_fn::<F,N,_>(h2f)
}

pub fn xof_hash_to_field<H,F,const N: usize>(
    sec_param: u16,
    dst: &[u8],
    msg: &[u8]
) -> [F; N]
